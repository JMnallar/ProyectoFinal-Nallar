...
const allPlaylists = document.querySelectorAll('.card-spotify-playlists, .card-concentracion');

searchInput.addEventListener('input', function() {
    const searchTerm = searchInput.value.trim().toLowerCase();
    
    allPlaylists.forEach(function(playlist) {
        const playlistTitle = playlist.querySelector('h2').innerText.toLowerCase();
